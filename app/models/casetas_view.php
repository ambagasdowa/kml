<?php
/**

		* 

		* PHP versions 4 and 5 

		* 

		* kml : Kamila Software 

		* Licensed under The MIT License  

		* Redistributions of files must retain the above copyright notice. 

		* 

		* @copyright     Jesus Baizabal 

		* @link          http://baizabal.xyz 

		* @mail	     baizabal.jesus@gmail.com 

		* @package       cake 

		* @subpackage    cake.cake.console.libs.templates.views 

		* @since         CakePHP(tm) v 1.2.0.5234 

		* @license       MIT License (http://www.opensource.org/licenses/mit-license.php) 

		*/
?>
<?php
class CasetasView extends AppModel {
	var $name = 'CasetasView';
	var $useDbConfig = 'mssql_sistemas';
	var $displayField = 'id';
	//The Associations below have been created with all possible keys, those that are not needed can be removed

	var $belongsTo = array(
		'CasetasHistoricalConciliations' => array(
			'className' => 'CasetasHistoricalConciliation',
			'foreignKey' => 'casetas_historical_conciliations_id',
			'conditions' => '',
			'fields' => '',
			'order' => ''
		),
		'CasetasControlsFiles' => array(
			'className' => 'CasetasControlsFile',
			'foreignKey' => 'casetas_controls_files_id',
			'conditions' => '',
			'fields' => '',
			'order' => ''
		),
		'CasetasStandings' => array(
			'className' => 'CasetasStanding',
			'foreignKey' => 'casetas_standings_id',
			'conditions' => '',
			'fields' => '',
			'order' => ''
		),
		'CasetasParents' => array(
			'className' => 'CasetasParent',
			'foreignKey' => 'casetas_parents_id',
			'conditions' => '',
			'fields' => '',
			'order' => ''
		)
	);

	var $hasMany = array(
		'CasetasControlsEvent' => array(
			'className' => 'CasetasControlsEvent',
			'foreignKey' => 'casetas_view_id',
			'dependent' => false,
			'conditions' => '',
			'fields' => '',
			'order' => '',
			'limit' => '',
			'offset' => '',
			'exclusive' => '',
			'finderQuery' => '',
			'counterQuery' => ''
		),
		'CasetasPending' => array(
			'className' => 'CasetasPending',
			'foreignKey' => 'casetas_view_id',
			'dependent' => false,
			'conditions' => '',
			'fields' => '',
			'order' => '',
			'limit' => '',
			'offset' => '',
			'exclusive' => '',
			'finderQuery' => '',
			'counterQuery' => ''
		),
		'CasetasConciliation' => array(
			'className' => 'CasetasConciliation',
			'foreignKey' => 'casetas_view_id',
			'dependent' => false,
			'conditions' => '',
			'fields' => '',
			'order' => '',
			'limit' => '',
			'offset' => '',
			'exclusive' => '',
			'finderQuery' => '',
			'counterQuery' => ''
		),
		'CasetasNotConciliation' => array(
			'className' => 'CasetasNotConciliation',
			'foreignKey' => 'casetas_view_id',
			'dependent' => false,
			'conditions' => '',
			'fields' => '',
			'order' => '',
			'limit' => '',
			'offset' => '',
			'exclusive' => '',
			'finderQuery' => '',
			'counterQuery' => ''
		),
// 		'CasetasLog' => array(
// 			'className' => 'CasetasLog',
// 			'foreignKey' => 'casetas_view_id',
// 			'dependent' => false,
// 			'conditions' => '',
// 			'fields' => '',
// 			'order' => '',
// 			'limit' => '',
// 			'offset' => '',
// 			'exclusive' => '',
// 			'finderQuery' => '',
// 			'counterQuery' => ''
// 		)
	);

}

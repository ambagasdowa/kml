				...
				$(".search_udn").select2();
					...
					$("#send_query").on('click', function(event) {
								...
								$( ".updateSearchResult" ).load(urlStruct,function(responseText, statusText, xhr) {
									...
									//
									// var subto = sub_zero(row, data, start, end, display);
									// options_datatable_disp_rend = {
									//       footerCallback:subto
									//   };
									// var rend_det = Object.assign( {}, options_datatable, options_datatable_disp_rend );

									var table_b = $('#table_det').DataTable(
										Object.assign( {}, options_datatable, calculate_row_test )
									 );
									// End table
